...
static bool importFunctions(const char *argv0, Module &DestModule) {
  ...
  auto CachedModuleLoader = [&](StringRef Identifier) {
    return ModuleLoaderCache.takeModule(Identifier.str());
  };
  FunctionImporter Importer(*Index, CachedModuleLoader, true);
...
static bool linkFiles(const char *argv0, LLVMContext &Context, Linker &L,
  ...
  for (const auto &File : Files) {
    ...
    if (!SummaryIndex.empty()) {
      ...
      // Promotion
      if (renameModuleForThinLTO(*M, *Index, true))
        return true;
    }
...
int main(int argc, char **argv) {
  ...
  auto Composite = std::make_unique<Module>("llvm-link-cudafe", Context);
  Linker L(*Composite);
